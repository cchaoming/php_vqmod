...
abstract class VQMod
{
    ...
    public static $_vqversion = '2.6.9';                        // Current version number
...
class VQModObject
{
    ...
    public function applyMod($mods, &$data)
    {
        ...
        foreach ($mods as $mod) {
            //覆盖文件优先
            if (!empty($mod['overfile'])) {
                $over_content = file_get_contents($mod['overfile']);
                if (empty($over_content)) {
                    VQMod::$log->write('VQModObject::applyMod - ERROR LOADING FILE: ' . $mod['overfile'], $this);
                }
                $tmp = $over_content;
                continue;
            }
            ...
            $global_search = isset($mod['search']->global_search) && $mod['search']->global_search == 'true' ? true : false;
            if ($global_search) {
                $changed = false;
                if ($mod['search']->regex == 'true') {
                    $pos = @preg_match($mod['search']->getContent(), $tmp);
                    if ($pos === false) {
                ...
                if ($pos !== false) {
                    $changed = true;
                    if ($mod['search']->regex == 'true') {
                        $tmp = preg_replace($mod['search']->getContent(), $mod['add']->getContent(), $tmp);
                    } else {
                        $tmp = str_replace($mod['search']->getContent(), $mod['add']->getContent(), $tmp);
                    }
                }
        ...
        foreach ($files as $file) {
            ...
            foreach ($filesToMod as $filename) {
                ...
                foreach ($operations as $opIndex => $operation) {
                    ...
                    $overwrite = $operation->getElementsByTagName('overwrite')->item(0);
                    if ($overwrite) {
                        $overPath = $path . $overwrite->nodeValue;
                        $overwrite_fullpath = VQMod::path($overPath);
                        if (!$skipOperation && is_file($overwrite_fullpath)) {
                            $this->mods[$fullPath][] = array(
                                'overfile' => $overwrite_fullpath
                            );
                        } else {
                            echo $overPath . PHP_EOL . $overwrite_fullpath;
                            VQMod::$log->write('Overfile not found: ' . $overwrite_fullpath, $this);
                        }
                        continue;
                    }
